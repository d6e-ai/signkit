-- Audit hash v2. Existing rows receive hash_version = 1 so legacy preimages
-- still verify. Recreated command triggers stamp hash_version = 2 on every new
-- audit_event insert.

ALTER TABLE audit_event ADD COLUMN hash_version INTEGER NOT NULL DEFAULT 1;

-- SQLite has no ALTER COLUMN ... SET DEFAULT. New application INSERTs and the
-- recreated triggers below write hash_version = 2 explicitly.

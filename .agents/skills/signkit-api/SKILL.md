...
- **Narrow Management Refusal:** Presenting a well-formed `signkit_` bearer to management surfaces (`/api/v1/api-keys/**` or `/api/v1/instance/**`, including `/api/v1/instance/bootstrap`) is refused outright with `403 Forbidden` (`urn:signkit:problem:api-key-not-permitted`), and any accompanying cookie session is suppressed to prevent self-escalation. Bootstrap is cookie-session-only — there is no deployment secret gating it — so the first authenticated identity to reach an empty instance claims the sole `owner` slot; it additionally requires the session's `email_verified` claim to be `true`, exactly like instance invitation acceptance, returning `403` `urn:signkit:problem:email-verification-required` otherwise.

...
`migrations/postgres` and `migrations/d1` are maintained as separate dialect-specific sets behind the same ports. A schema change usually means writing both, plus the matching migration specs. The shared model avoids database enums, arrays, and dialect-specific JSON column types. Tenant-owned tables carry `organization_id` in their keys and foreign keys; instance-scoped `instance_member` / `instance_bootstrap` / `api_key` tables do not — see [architecture/persistence.md § Persistence](architecture/persistence.md#persistence). `api_key_organization_grant` and its two command receipts are the deliberate exception: they are instance-scoped rows that name an organization, because bridging a local key to an external d6e tenant is exactly their purpose. Before v0.1.0, a baseline migration was rewritten in place rather than followed by a compatibility migration; released migrations are additive and must remain backward-compatible with the previous released Worker. Rewriting one invalidates every database that already applied the old text: each pre-release D1, local **and** remote, must be reset and recreated from the current migration set — `wrangler d1 migrations apply` will not repair an already-applied baseline, and a database left on the old text keeps the old constraints while its `d1_migrations` ledger claims to be current. Resetting was only acceptable because those instances were pre-release and empty; verify it holds no data worth keeping before you do it.
...
Tag workflow `.github/workflows/release-cloudflare-bundle.yml` runs on `v*` tags as a draft-first full release: it enforces tag-equals-version across the root package, create-signkit, and the Rust CLI, builds and smoke-checks the Node profile, builds and verifies the Cloudflare bundle, builds the Rust `--locked --release` binary and the Docker image, regenerates a unified `.release/assets/SHA256SUMS` over every asset, verifies it, and uploads all assets to a DRAFT GitHub Release. Only then may job `publish-npm` run `npm publish` for create-signkit with a version-checked npm CLI >= 11.5.1 installed into an isolated prefix (OIDC Trusted Publishing with optional `NODE_AUTH_TOKEN`; pnpm is used for workspace install and to invoke that npm CLI). Job `publish-release` marks the draft public only after `publish-npm` succeeds, so a tag never leaves a public release behind an unpublished npm package; perfect cross-registry atomicity is impossible and recovery is documented in the workflow header. Prerelease tags publish to npm dist-tag `beta`; stable tags publish to `latest`. The first npm publication of the still-unpublished package needs a short-lived token or a manual publish before Trusted Publisher can be attached. See [release-checklist.md](release-checklist.md) for the non-publishing local verification that precedes any tag push.

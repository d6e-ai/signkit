...
| `SIGNKIT_BOOTSTRAP_SECRET`                                                                                 | High-entropy bearer secret (min 32 printable ASCII chars) gating `POST /api/v1/instance/bootstrap`, the one-time instance owner claim. Unset or malformed values fail closed with an opaque 404. |

...
| `SIGNKIT_BOOTSTRAP_OWNER_EMAIL`                                                                                   | Required on every uninitialized non-local instance. Restricts instance bootstrap to one verified email; see [Claim the initial owner immediately after deploy](#claim-the-initial-owner-immediately-after-deploy). Not a secret; the local-development-only `SIGNKIT_ALLOW_UNSAFE_FIRST_USER_BOOTSTRAP=true` opt-in replaces it on loopback origins. |
| `SIGNKIT_ALLOW_UNSAFE_FIRST_USER_BOOTSTRAP`                                                                       | Exactly `true` restores first-user-wins bootstrap, but only in local development (Node with a loopback `SIGNKIT_PUBLIC_ORIGIN`). Ignored on Cloudflare Workers, Vercel, Node production, and any non-loopback origin. Never set in production.                                                                                                       |
...
The unsafe opt-in is honored only in local development: a Node process whose `SIGNKIT_PUBLIC_ORIGIN` names a loopback host (`localhost`, `127.0.0.1`, `::1`). The exact `true` flag is ignored on Cloudflare Workers (including `wrangler dev`), on Vercel, in Node production, and with any non-loopback or missing origin — those environments fail closed without `SIGNKIT_BOOTSTRAP_OWNER_EMAIL`. Never set the unsafe flag in production. `create-signkit --cloudflare deploy` requires the equivalent `--bootstrap-owner-email` flag and applies it as a non-secret Worker var.

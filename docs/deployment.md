...
`wrangler.jsonc` declares the `DB` (D1), `OBJECTS` (R2), `ASSETS`, and `EMAIL` bindings, `nodejs_compat`, a default `SIGNKIT_MAIL_PROVIDER=cloudflare` var, observability, and a `* * * * *` cron trigger. R2 is used through its in-process binding rather than an S3 endpoint, and D1 migrations live in `migrations/d1`.

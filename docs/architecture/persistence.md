...
immutable completion PDF. Completion-PDF publication persists its generated byte size alongside
the object key and digest; enqueue joins all three values rather than trusting a caller assertion.
Legacy rows whose size is unknown remain readable but are not sealable. The job freezes the source

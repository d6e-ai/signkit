...
The embedded timestamp is metadata and a coarse ordering hint only. It is never authorization, never trusted event time, and never a substitute for audit ordering: the audit chain's own `sequence`, `occurred_at`, and hash links remain the sole ordering and integrity evidence.
...
- external d6e-auth user identifiers, which SignKit stores as identity references;

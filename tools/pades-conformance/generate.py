...
def issue_certificates(
...
) -> tuple[Path, Path, Path, x509.Certificate, x509.Certificate]:
    ...
    return signer_key_path, signer_path, tsa_key_path, signer, tsa
...
def assert_signed_structure(source: bytes, signed: bytes, *, timestamped: bool) -> None:
    ...
    has_timestamp = unsigned_attributes is not None and any(
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="signkit-pades-private-") as private_name:
        ...
        signer_key_path, signer_path, tsa_key_path, signer_cert, tsa_cert = (
        ...
        assert_signed_structure(source, bt, timestamped=True)
    ...
    manifest = {
        ...
        "certificates": {
            ...
            "tsaSha256": tsa_cert.fingerprint(hashes.SHA256()).hex(),
        },
        "files": {

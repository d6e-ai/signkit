...
/**
 * Bounded structural PDF parser. It counts page-tree leaves and reads the
 * first page's MediaBox; it never decodes a content stream or renders.
 */
...
type PdfValue = number | boolean | null | string | PdfName | PdfRef | PdfDict | PdfArray;
...
export function parsePdfPageMetadata(bytes: Uint8Array): PdfPageMetadata {
	return new PdfPageMetadataParser(bytes).parse();
}

class PdfPageMetadataParser {
	...
	parse(): PdfPageMetadata {
		...
		this.#walkPageTree(pagesRef, null, null, 0, new Set(), pages);
	...
	#walkPageTree(
		...
		if (isName(type, 'Page')) {
			pages.push(this.#pageInfo(mediaBox, rotate));
		...
		for (const kid of kids.items) {
			this.#walkPageTree(kid, mediaBox, rotate, depth + 1, visited, pages);
		}
		if (isRef(nodeValue)) visited.delete(nodeValue.objectNumber);
	}

	#pageInfo(mediaBox: PdfArray | null, rotate: number | null): PageInfo {
	...
	#parseStreamObject(streamObjectNumber: number): readonly PdfValue[] {
		...
		const parser: PdfPageMetadataParser = new PdfPageMetadataParser(decoded);
	...
	#parseNumberOrRef(): PdfValue {
		...
		if (
			...
			const secondStart: number = this.#pos;
...
function isNameValue(value: PdfValue): value is PdfName {
	return value !== null && typeof value === 'object' && value.kind === 'name';
}

function isName(value: PdfValue | undefined, expected: string): boolean {
	return value !== undefined && isNameValue(value) && value.value === expected;
}

	...
	sqlite
		.prepare(
			`INSERT INTO instance_member (user_id, role, status, created_at, updated_at)
			 VALUES (?, 'owner', 'active', '2026-09-12T00:00:00.000Z', '2026-09-12T00:00:00.000Z')`
		)
		.run(USER_ID);
	sqlite
		.prepare(
			`INSERT INTO envelope (id, created_by_user_id, title, status, created_at, updated_at)
			 VALUES (?, ?, 'Agreement', 'draft', '2026-09-12T00:00:00.000Z', '2026-09-12T00:00:00.000Z')`
		)
		.run(ENVELOPE_ID, USER_ID);
...
describe('D1EnvelopeDocumentStore', () => {
	it('derives titles for newly observed paths and orders by input order', async () => {
		...
		const result = await store.sync(ENVELOPE_ID, [
		...
		expect(result.every((document) => document.envelopeId === ENVELOPE_ID)).toBe(true);
	});
	...
	it('preserves stable ids and titles across a re-sync that reorders and drops a path', async () => {
		...
		await store.renameDocument(ENVELOPE_ID, 'documents/agreement.md', 'Master Services Agreement');
	...
	it('renameDocument returns null for an untracked path', async () => {
		const { d1 } = database();
		const store = new D1EnvelopeDocumentStore(d1);
		const result = await store.renameDocument(ENVELOPE_ID, 'documents/missing.md', 'Title');

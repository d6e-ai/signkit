...
import { OrphanCollector } from './orphan-collector';

const privateEnv = vi.hoisted<Record<string, string | undefined>>(() => ({}));

vi.mock('$env/dynamic/private', () => ({ env: privateEnv }));

import { resolveOrphanCollector } from './orphan-collector-runtime';

afterEach((): void => {
	for (const key of Object.keys(privateEnv)) delete privateEnv[key];
});

describe('resolveOrphanCollector', () => {
	it('returns null when no durable Node configuration exists', async () => {
		await expect(resolveOrphanCollector({})).resolves.toBeNull();
	});
	...
	it('fails closed on an incomplete Cloudflare binding set without using S3', async () => {
		...
		await expect(resolveOrphanCollector({ platform })).resolves.toBeNull();
	});
	...
	it('uses native D1 and R2 bindings when both are present', async () => {
		...
		const collector = await resolveOrphanCollector({ platform });
		expect(collector).toBeInstanceOf(OrphanCollector);
	});
	...
	it('rejects partial Node configuration without exposing its values', async () => {
		...
		await expect(resolveOrphanCollector({})).rejects.toThrow(
			'S3_REGION is required for S3 draft persistence'
		);
	});

	it('constructs PostgreSQL and S3 collection from a complete Node configuration', async () => {
		setCompleteNodeConfiguration();

...
describe('verifyCompletionAuditChain', () => {
	...
	it('fails closed when a terminal recipient event does not end the chain with recipient actor type', async () => {
		...
		// A tampered actor_type does not change the recomputed hash for
		// generic events, so this must be caught by the explicit actor-type
		// check, not by hash verification.
...
describe('auditEventHashPreimage', () => {
	...
	// hash.
	it('hashes the draft revision shape with actor type and actor id included, in exact key order', async () => {
	...
	it('hashes the generic shape without actor type, in exact key order', async () => {
		const events = await buildVerifiedAuditChain(CONTEXT, steps(2));

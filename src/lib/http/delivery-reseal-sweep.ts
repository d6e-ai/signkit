...
import type {
	DeliveryResealSweepResult,
	DeliveryResealSweepService
} from '$lib/application/delivery/reseal-sweep-service';
...
export type DeliveryResealSweepServiceResolver = (
	context: ResolverContext
) => DeliveryResealSweepService | null | Promise<DeliveryResealSweepService | null>;

/**
 * Maintenance endpoint, not part of the hot delivery path: migrates
 * outstanding (non-`processing`) delivery outbox ciphertext off a retiring
 * key onto the active one. Reuses the `DELIVERY_WORKER_SECRET` constant-time
 * bearer convention rather than a new credential.
 */
export function createDeliveryResealSweepHandler(
	resolveService: DeliveryResealSweepServiceResolver,
	...
	return async ({ platform, request, url }): Promise<Response> => {
		...
		let service: DeliveryResealSweepService | null;
		try {
			service = await resolveService({ platform });
		} catch (error: unknown) {
			logResealSweepError('delivery_reseal_sweep_resolution_failed', error);
			service = null;
		}
		if (service === null) {
			return problemResponse({
				...
				status: 503,
				detail: 'The delivery store and encryption key must be configured.',
				instance: url.pathname
			});
		}

		try {
			const result: DeliveryResealSweepResult =
				await service.resealOutstandingCapabilities(RESEAL_SWEEP_BATCH_LIMIT);
			return Response.json(result, {
				status: 200,
				headers: { 'cache-control': 'no-store' }
			});
		} catch (error: unknown) {
			logResealSweepError('delivery_reseal_sweep_failed', error);
			return problemResponse({

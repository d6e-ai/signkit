...
#[command(
    ...
    long_about = "Non-interactive, agent-first CLI for SignKit. Sender commands use the scoped instance-member API key; recipient commands require that recipient's separate invitation capability and explicit consent for mutations. Key management and instance administration require interactive operator sessions and are not exposed here.",
...
pub struct EnvelopeExportDocxArgs {
    ...
    /// Destination regular file. Document bytes are never written to stdout.
    #[arg(long, value_name = "PATH")]
    pub output: String,
}
